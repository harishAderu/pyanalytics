...
df.custname.value_counts().sort_values(ascending=True)[0:8]
...
df.groupby('custname').agg({'revenue':np.sum , 'custname':np.count_nonzero})
df.groupby('custname').aggregate({'revenue':np.sum}).sort_values(by='revenue', ascending=True)
